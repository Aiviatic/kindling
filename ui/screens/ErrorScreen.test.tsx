...
describe('<ErrorScreen>', () => {
  ...
  it('is announced assertively (role="alert")', () => {
    renderError(failEvent(StepId.InstallFramework, 'boom', ErrorCode.ExecFailed));
  ...
  it('offers Retry for a generic failure (Flow re-runs the failed step)', () => {
    const onRetry = vi.fn();
    renderError(failEvent(StepId.InstallFramework, 'boom', ErrorCode.ExecFailed), { onRetry });
    fireEvent.click(screen.getByRole('button', { name: 'Retry' }));
    expect(onRetry).toHaveBeenCalledTimes(1);
  });

  it('renders the dedicated BMad-install-failure guidance', () => {
    renderError(failEvent(StepId.InstallFramework, 'exit 1', ErrorCode.BmadInstallFailed));
  ...
  it('falls back to the engine message when the failure carries no code', () => {
    renderError(failEvent(StepId.InstallFramework, 'A specific engine-authored failure message.'));
