...
describe('writeFailureLog', () => {
  it('writes a report file and returns its path', async () => {
    const path = await writeFailureLog(
      { step: StepId.InstallBmad, error: 'boom\n  at x', events },
    ...
    expect(body).toContain('Failed step: install.bmad');
  ...
  it('does not leak the process environment (no secrets)', async () => {
    ...
    try {
      const path = await writeFailureLog({ step: StepId.InstallBmad, error: 'e', events }, { dir: tmp });
      const body = await readFile(path, 'utf8');
